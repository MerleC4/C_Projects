...
void postOrder(targetNode *target) {
	...
	target->visited = 1;
	struct stat status;
	int result = stat(target->name, &status);
	if (result == -1) {
		if (target->target == 0) {
			fprintf(stderr, "Error: %s is nto a target. Exiting.\n", target->name);
			freeFile();
			exit(1);
		}
		else {
		...
		target->sec = status.st_mtim.tv_sec;
		target->nano = status.st_mtim.tv_nsec;
		target->exist = 1;
	}
	...
	while (curr) {
		...
		if (curr->tcurr->visited != 2) {
			fprintf(stderr, "Error: cyclical dependency found.\n");
		}
		else if (target->build == 0) {
			if (curr->tcurr->exist == 0 || curr->tcurr->sec > target->sec) {
				target->build = 1;
			}
			else if (curr->tcurr->sec == target->sec) {
				if (curr->tcurr->nano > target->nano) {
					target->build = 1;
				}
			}
		}
	...
	if (target->build == 1) {
		commandPrint(target);
		result = stat(target->name, &status);
		if (result != -1) {
			target->exist = 1;
			target->sec = status.st_mtim.tv_sec;
			target->nano = status.st_mtim.tv_nsec;
...
/*
* commandPrint(targetNode) -- This will print out an error command if any
...
void commandPrint(targetNode *target) {
	...
	while (curr) {
		printf("%s\n", curr->command);
		int result = system(curr->command);
		if (result != 0) {
